...
\newglossaryentry{MSL}{name=MSL,description={MyStatsLab: a commercial computer-based homework management system owned and distributed by Pearson \url{http://www.pearsonmylabandmastering.com/northamerica/mystatslab/}}}
...
% to do:
%   - shorten toc entries where necessary, indentation of toc, generally tidy it Alex
%   - check colons and semi-colons surrounding bullets Chris
%   - should seasons be capitalized or not? Alex
%   - make consistent choice about on-campus, oncampus, on campus, online (online and on-campus are the winners) Alex
%   - make consistent choice about full-time, full time, part-time, part time (full-time and part-time win) Alex
%   - change all 'adjunct' to 'part time' (or 'part-time') Alex
%   - make consistent choice about face 2 face or face-to-face (face-to-face wins, F2F can be used in tables; DL is fine anywhere) Alex
%   - check that all sections in the appendix are referenced in the body Chris
%       (use refcheck package to check the output and grep the log file) Chris
...
%   - check that no hyphens, em dashes, or en dashes have spaces before or after them (except hanging hyphens) Alex
%   - check that hyphens, em dashes, en dashes are used appropriately Alex
%   - are we doing titles? e.g Mr, Dr, Ms? should be consistent. Decision from Steve, Alex, Chris: no titles Alex
%   - should we link back from the appendices to where that appendix is menionted?
%   - remove refcheck package Chris
%   - subsections and sections should be distinguished in the body. Right now, both are bold, and when there is no text between \section{   } and \subsection{   } it is confusing
...
%\includeonly{appendices/demographicData}
%\includeonly{appendices/ALEKSpilot}
...
\include{appendices/courseScheduling}%Not referenced
  \fixthis{some appendices are not referenced}
\include{appendices/dlSuccessfulCompletion}%Not referenced
\include{appendices/enrollmentSummaries}%Not referenced
